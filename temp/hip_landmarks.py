""" hip landmark evaluation script"""
import math
import shutil
...
def visualize_mesh(nifti_filename, offscreen=False, screenshot_out_dir="."):
    ...
    sp_found = True
    try:
        sacral_region, sp = get_sacral_promontory(
...
def get_sacral_promontory(aligned_mesh_obj, is_1, is_2, msp_p1, msp_p2, asis_midpoint):
    ...
    sacral_region = (
        ...
        .cut_with_plane(left_sagittal, (1, 0, 0))
        .cut_with_plane(right_sagittal, (1, 0, 0), invert=True)
        .cut_with_plane(top_transverse, (0, -1, 0))
    )
    ...
    while math.isnan(sp_idx) and len(sacral_region.points()) != 0:
        sp, sp_idx = get_farthest_point_along_axis(
            sacral_region.points(), axis=2, negative=False
        )
...
def get_candidate_ischial_spines(ischial_mesh_left):
    ...
    for i in range(
        -5, 45, 1
    ):  # rotate mesh along x-axis by (-5,45) degree in stepsize of 1 degree
...
def get_landmarks(gt_filename, pca_aligned_mesh):
    ...
    TRANSVERSE_PLANE_ALPHA = 0.6
    transverse_plane_height_gt, distal_midpoint_gt = get_transverse_plane_height(
        pca_aligned_mesh, anterior_midpoint_gt, alpha=TRANSVERSE_PLANE_ALPHA
    )
    gt_points = get_asis_estimate(
        pca_aligned_mesh, transverse_plane_pos=(0, transverse_plane_height_gt, 0)
    )
    ...
    if anterior_midpoint_gt.GetPosition()[1] < asis_midpoint_gt.GetPosition()[1]:
        if "s0477" in gt_filename:
            ...
            gt_points = get_asis_estimate(
                pca_aligned_mesh,
                transverse_plane_pos=(0, transverse_plane_height_gt, 0),
            )

    # Sanity check: Pubic Tubercles(PT) should be closer to distal plane than ASIS
    asis_midpoint_gt = get_midpoint(gt_points[0], gt_points[1])
    gt_points = fine_tune_pt_after_sanity_check(
        gt_filename, pca_aligned_mesh, distal_midpoint_gt, gt_points, asis_midpoint_gt
    )
    return mwp_gt, transverse_plane_height_gt, distal_midpoint_gt, gt_points


def fine_tune_pt_after_sanity_check(
    filename, mesh_obj, distal_midpoint, asis_points, asis_midpoint, reduce_by=5
):
    ...
    if not pt_1_test_passed or not pt_2_test_passed:
        ...
        asis_points = get_asis_estimate(
            mesh_obj, (0, new_lower_transverse_plane_height, 0)
        )
    return asis_points
...
def process_runs():
    ...
    for f in get_files_from_run_id(run_id=run_id, suffix_regex="*.nii.gz"):
        visualize_mesh(
            nifti_filename=str(f),
            offscreen=True,
            screenshot_out_dir=f"2d-3d-benchmark/{run_id}/visualization_v4",
        )
