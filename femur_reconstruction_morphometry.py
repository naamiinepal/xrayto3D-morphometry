...
from xrayto3d_morphometry import (cma_es_search_candidate_cut_plane,
                                  ...
                                  get_nifti_stem, get_points_along_directions,
                                  get_vector_from_points)


def get_femur_morphometry_for_model_predictions(nifti_filename,diaphysis_center, robust=False,visualize=True,offscreen=True,screenshot_out_dir='.'):
...
def get_diaphysis_center_from_groundtruth(predicted_nifti_file):
    ...
    manual_localization_file = list(Path('femur_manual_cut_plane').glob(f'{file_prefix}*_gt.nii.gz'))
    assert len(manual_localization_file) == 1, f'Expected 1 but got {len(manual_localization_file)} files with prefix {file_prefix}'
    ...
    subtroc_mesh = extract_volume_surface(
            get_segmentation_volume(
                str(manual_localization_file[0]),
                label_dict['sub_troc']
            )
        )
...
def get_predictions_from_run_id(run_id:str,ground_truth_nifti_file):
    ...
    predicted_nifti_file = list(Path(f'2d-3d-benchmark/{run_id}/evaluation').glob(f'{file_prefix}*_pred.nii.gz'))
    assert len(predicted_nifti_file) == 1, f'Expected 1 but got {len(predicted_nifti_file)} files with prefix {file_prefix}'
...
def process_dir():
    ...
    filestream = open(Path(f'2d-3d-benchmark/{args.run_id}/evaluation')/'morphometry-log.csv','w')
    ...
    for f in tqdm(filenames):
        ...
        screenshot_out_dir = Path(f'2d-3d-benchmark/{args.run_id}/evaluation/visualize-v2')
        screenshot_out_dir.mkdir(exist_ok=True)
        metric_out = get_femur_morphometry_for_model_predictions(str(prediction_nifti_file),diaphysis_center,robust=args.robust,visualize=True,offscreen=True,screenshot_out_dir=str(screenshot_out_dir))        
...
def test_single():
    ...
    get_femur_morphometry_for_model_predictions(args.nifti_file,diaphysis_center,robust=args.robust,visualize=True,offscreen=False)
