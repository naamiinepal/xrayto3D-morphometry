...
def process_femur(nifti_filename, offscreen=False, screenshot_out_dir="."):
    ...
    initial_diaphysis_cpn, csa = grid_search_candidate_cut_plane(
        mesh_obj, diaphysis_com.GetPosition(), (0, 1, 0), verbose=True
    )
    ...
    # neck axis
    neck_mesh = extract_volume_surface(
        get_segmentation_volume(nifti_filename, label_dict["neck"])
    )
    neck_com = vedo.Point(neck_mesh.center_of_mass())
    initial_neck_cpn, csa = grid_search_candidate_cut_plane(
        mesh_obj, neck_com.GetPosition(), (1, 0, 0), verbose=True
    )
    ...
    # head sphere
    head_mesh = extract_volume_surface(
        get_segmentation_volume(nifti_filename, label_dict["head"])
    )
    ...
    vedo.show(
        ...
        diaphysis_com,
        mesh_obj.clone()
        .cut_with_plane(diaphysis_com.GetPosition(), initial_diaphysis_cpn)
        .boundaries(),
        get_arrow_actor(diaphysis_com.GetPosition(), initial_diaphysis_cpn).opacity(
            0.3
        ),
        ...
        neck_com,
        mesh_obj.clone()
        .cut_with_plane(neck_com.GetPosition(), initial_neck_cpn)
        .boundaries(),
...
if __name__ == "__main__":
    ...
    process_femur(
        nifti_filename="femur_manual_cut_plane/s0104_femur_left_msk_gt.nii.gz",
        offscreen=False,
        screenshot_out_dir="test_data",
    )
    # run_id = 'ceio7qj7'
