...
from xrayto3d_morphometry import (
    LandmarkNotFoundError, align_along_principal_axes,
    brute_force_search_get_closest_points_between_point_clouds,
    get_asis_estimate, get_farthest_point_along_axis, get_ischial_mesh_cut,
    get_maximal_pelvic_width, get_mesh_from_segmentation, get_midpoint,
    get_psis_estimate, get_transverse_plane_height, lerp, move_to_origin,
    get_files_from_run_id, get_oriented_camera)
...
def get_sacral_promontory(aligned_mesh_obj, is_1, is_2, msp_p1, msp_p2, asis_midpoint):
    ...
    sacral_region = aligned_mesh_obj.clone(transformed=True).cut_with_plane(left_sagittal, (1, 0, 0)).cut_with_plane(
        right_sagittal, (1, 0, 0), invert=True).cut_with_plane(top_transverse, (0, -1, 0))
...
def pt_sanity_test(filename, distal_midpoint, asis_midpoint, pt):
    ...
    if dist_pt_distal > dist_pt_asis:
        test_passed = False
        print(test_passed, Path(filename).stem, 'PT distance to distal ',
              dist_pt_distal, 'distance to asis ', dist_pt_asis)
...
def process_runs():
    ...
    for f in get_files_from_run_id(run_id=run_id, suffix_regex='*.nii.gz'):
        visualize_mesh(nifti_filename=str(f), offscreen=True,
