...
def get_symmetry_plane(vert_mesh):
    ...
    sym_plane = vedo.fit_plane(avg_points)
    return sym_plane
...
sample_file = "test_data/sub-verse004_vert-24-seg-vert_msk.nii.gz"

vert_mesh = get_mesh_from_segmentation(sample_file,largest_component=True)
...
sym_plane_boundaries = vert_mesh.clone(transformed=True).cut_with_plane(normal=(sym_plane.normal)).boundaries()
...
vedo.show(
    # vert_mesh.c("white", 0.2),
    vert_mesh.cut_with_plane(normal=sym_plane.normal,invert=True).c('white',alpha=0.5),
    ...
    vedo.Line(sp_axis1),
    vedo.Line(v0, s0),
    a_bs.lw(10), a_bi.lw(10),
    # a_ba.lw(10), a_bp.lw(10),
    ...
    # vedo.Points(anterior_up_proj, r=4, c='red'),
    # vedo.Points(anterior_down_proj, r=4, c='green'),
    # vedo.Point(anterior_down_most_proj, r=12, c='white'),
    # vedo.Point(anterior_up_most_proj, r=12, c='white'),
    a_ba.lw(10), a_bp.lw(10),
    axes=1,
    camera=topview_cam,
    resetcam=False
