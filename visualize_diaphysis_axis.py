...
def main():
    sample_fp = 'test_data/s0000_femur_left_msk_detailed_4class.nii.gz'
    mesh_obj = get_mesh_from_segmentation(sample_fp,largest_component=False)

    label_dict = {'head':1,'neck':2,'intra_troc':3,'sub_troc':4}
    subtroc_mesh = extract_volume_surface(get_segmentation_volume(sample_fp,label_dict['sub_troc']))
    # diaphysis axis
    ...
    # neck axis
    neck_mesh = extract_volume_surface(get_segmentation_volume(sample_fp,label_dict['neck']))
    ...
    # head sphere
    head_mesh = extract_volume_surface(get_segmentation_volume(sample_fp,label_dict['head']))
