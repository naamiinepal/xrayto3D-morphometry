...
def get_femur_morphometry(nifti_filename, visualize=True, screenshot=True, offscreen=True, screenshot_out_dir='.'):
    ...
    diaphysis_axis, _ = grid_search_candidate_cut_plane(mesh_obj, diaphysis_center.GetPosition(), (0, 1, 0))
    ...
    additional_diaphysis_axis = [grid_search_candidate_cut_plane(
        mesh_obj, center, diaphysis_axis, range_min=-0.1, range_max=0.1) for center in additional_diaphysis_center]
    ...
    additional_l_a = [get_line_segment(center, axis, 400) for center, (axis, _)
                      in zip(additional_diaphysis_center, additional_diaphysis_axis)]
    ...
    additional_neck_pi_n = np.asarray([grid_search_candidate_cut_plane(
        mesh_obj, com, pi_n, range_min=-0.1, range_max=0.1, verbose=False) for com in additional_p_n])
    min_index = np.argmin(additional_neck_pi_n[:, 1])
    min_p_n = additional_p_n[min_index]
    min_pi_n = additional_neck_pi_n[min_index, 0]
    neck_center, neck_radius, neck_normal = vedo.fit_circle(
    ...
    additional_diaphysis_directions = [get_vector_from_points(
        *get_line_segment(p, n, 10)) for p, (n, _) in zip(additional_diaphysis_center, additional_diaphysis_axis)]
    ...
    additional_neck_directions = [get_vector_from_points(
        *get_line_segment(p, n, 10)) for p, (n, _) in zip(additional_p_n, additional_neck_pi_n)]
    ...
    additional_femoral_neck_angles = [get_angle_between_vectors(d, n) for d, n in zip(
        additional_diaphysis_directions, additional_neck_directions)]
    ...
    additional_femoral_head_offset = [get_distance_to_line_segment(
        head_sphere.center.tolist(), *l) / 2 for l in additional_l_a]
    mean_fho = np.mean(additional_femoral_head_offset)
    std_fho = np.std(additional_femoral_head_offset)

    print(f'Femoral Diaphysis axis {ct_x_angle:.2f} {ct_y_angle:.2f} {ct_z_angle:.2f} {np.mean(additional_ct_x_angles):.2f}+/-{np.std(additional_ct_x_angles):.2f} {np.mean(additional_ct_y_angles):.2f}+/-{np.std(additional_ct_y_angles):.2f} {np.mean(additional_ct_z_angles):.2f}+/-{np.std(additional_ct_z_angles):.2f}')

    print(f'Femoral Neck axis {neck_x_angle:.2f} {neck_y_angle:.2f} {neck_z_angle:.2f} {np.mean(additional_neck_x_angles):.2f}+/-{np.std(additional_neck_x_angles):.2f} {np.mean(additional_neck_y_angles):.2f}+/-{np.std(additional_neck_y_angles):.2f} {np.mean(additional_neck_z_angles):.2f}+/-{np.std(additional_neck_z_angles):.2f}')

    print(f'Femoral Head  Radius {femoral_head_radius:.2f} +/- {head_sphere.residue:.2f}')
    print(f'Femoral Neck Angle {180.0 - femoral_neck_angle:.2f} {180.0 - np.mean(additional_femoral_neck_angles):.2f} +/- {np.std(additional_femoral_neck_angles):.2f}')
...
def test_single():
    ...
    get_femur_morphometry(args.nifti_file, visualize=args.visualize,
                          offscreen=args.offscreen, screenshot=args.screenshot)
...
def process_dir():
    ...
    header = ['subject-id', 'FHR(mm)', 'FNA(degrees)', 'FNW(mm)',  'FHO(mm)', 'FHR(std)', 'FNA(std)', 'FHO(std)']
    filestream_writer.writerow(header)

    for f in tqdm(filenames):
        metric_out = get_femur_morphometry(str(f), visualize=True, offscreen=True,
                                           screenshot=True, screenshot_out_dir='femur_manual_cut_plane/visualize')
